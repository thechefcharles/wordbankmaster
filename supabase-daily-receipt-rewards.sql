-- ============================================================================
-- Daily deposit slip: also surface today's attendance (show-up) reward.
-- ============================================================================
-- Attendance ($50 + streak-milestone bonuses) is credited at daily_start, so
-- by the time the deposit slip renders it's already in the balance. Read it
-- back from today's ledger row so the slip can show the full money story
-- (Starting Balance → +Show-up bonus → +Deposit → −Loan repayment). Robust
-- across resume since it reads the ledger, not transient session state.
-- ============================================================================
BEGIN;
...
DECLARE s public.daily_sessions; v_won BOOLEAN; v_board JSONB; v_base INT; v_kept INT; v_mult NUMERIC; v_winnings INT;
  v_loan BIGINT; v_loan_repaid BIGINT := 0; v_attendance BIGINT := 0;
BEGIN
