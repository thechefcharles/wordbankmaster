...
mkdirSync('screenshots/settings',{recursive:true});
...
try{
  sql(`UPDATE profiles SET username=COALESCE(username,'pwtest_wb'), avatar='{"top":"shortFlat","clothing":"hoodie","clothesColor":"5199e4"}'::jsonb WHERE id=(SELECT id FROM auth.users WHERE email='${EMAIL}')`);
  ...
  console.log('sections:', await p.locator('.set-label').count(), '| groups:', await p.locator('.set-group').count(), '| rows:', await p.locator('.set-row').count());
  await p.screenshot({path:'screenshots/settings/01-top.png'});
  await p.locator('.set-group').last().scrollIntoViewIfNeeded().catch(()=>{}); await wait(300);
  await p.screenshot({path:'screenshots/settings/02-bottom.png'});
  console.log('done');
