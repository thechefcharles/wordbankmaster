// Screenshot the full-body avatar (builder hero) + head-crop profile icon.
...
try {
  ...
  // give them a saved look so the menu icon + profile render an avatar
  sql(`UPDATE profiles SET username=COALESCE(username,'pwtest_wb'), avatar='{"skinColor":"edb98a","top":"bigHair","hairColor":"c93305","eyes":"happy","eyebrows":"raisedExcited","mouth":"smile","clothing":"hoodie","clothesColor":"ff488e","accessories":"sunglasses","facialHair":"none"}'::jsonb WHERE id='${uid}'`);
  ...
  await p.screenshot({ path: 'screenshots/avatar/10-menu-head-icon.png' });
  console.log('account icon head svg:', await p.locator('.account-ic svg').count());
  await p.goto(BASE + '/avatar', { waitUntil: 'domcontentloaded' }); await wait(1500);
  console.log('full-body hero present:', await p.locator('.wb-avatar-full').count());
  await p.screenshot({ path: 'screenshots/avatar/11-fullbody.png' });
  console.log('done');
