-- Item roster rebalance (2026-07-18).
-- Cuts: Heat Shield, Free Skip, Overdrive (all run-rescue → pure solve-or-bust),
-- plus Free Vowel & Last Letters (weaker duplicate reveals). Enables the two Daily
-- Interest Boosts. Reprices the survivors up on the strong ones. Cash Game now caps
-- power-ups at one per puzzle, matching Challenges. (Tax → 3 buys ships separately.)

-- Retire the cut items (invisible in store + unbuyable; get_powerups/buy_powerup gate on active).
UPDATE public.powerups SET active = false
  WHERE id IN ('heat_shield','free_skip','overdrive','free_vowel','last_letters');

-- Turn the Daily Interest Boosts back on.
UPDATE public.powerups SET active = true
  WHERE id IN ('bounty_boost','jackpot_boost');

-- Reprice survivors to match the stakes (strong reveals up).
UPDATE public.powerups p SET price = v.price FROM (VALUES
  ('free_reveal', 100), ('extra_hint', 150), ('vowel_vision', 250), ('reveal_word', 300), ('half_off', 150),
  ('sabotage_toll', 80), ('sabotage_tax', 100), ('sabotage_fog', 120), ('sabotage_vowel_block', 120), ('sabotage_lock', 150)
) AS v(id, price) WHERE p.id = v.id;

-- Clear retired items from inventories so nothing dead lingers (pre-launch, small base).
DELETE FROM public.user_powerups_v2
  WHERE powerup_id IN ('heat_shield','free_skip','overdrive','free_vowel','last_letters');
