<script>
  ...
  // Ledger filters: by type, and sort by date/amount.
  let typeFilter = 'all';
  let sortBy = 'newest';
    ...
    <div class="led-head">
      ...
      {#if b.ledger.length}
        <div class="led-filters">
          <select class="led-sel" bind:value={typeFilter} aria-label="Filter by type">
            <option value="all">All types</option>
            {#each ledgerTypes as t}<option value={t}>{reasonLabel(t)}</option>{/each}
          </select>
          <select class="led-sel" bind:value={sortBy} aria-label="Sort">
...
<style>
  ...
  .led-sel { padding: 5px 8px; border-radius: 9px; border: 1px solid var(--border); background: var(--surface); color: var(--text); font-size: 0.74rem; font-weight: 600; max-width: 140px; }
