...
| **6 — Power-up overhaul** | Fairness split: **Daily** = one shared **Daily Modifier** identical for all, rotating by date (6a ✅). **Arcade** = per-run roguelike — earn one power-up per solve, spend within the run (6b ✅ engine; 6c ✅ tray UI: tappable chips w/ counts, armed "ON" effects, "Earned X" in the solve modal, Shield-saved bust message, arcade-aware keyboard prices). Roster: 🎟️ free_reveal, 💰 extra_bank, 🏷️ discount, 👁️ vowel_vision, 🛡️ insurance, 💎 double_payout, ⚡ multiplier_boost, 🔰 shield, ⏭️ skip, ❤️ extra_try. | Server + client | ✅ |
