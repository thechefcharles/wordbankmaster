-- Symmetric daily-streak column rename (internal only; output field names preserved).
BEGIN;
ALTER TABLE public.profiles RENAME COLUMN play_streak            TO current_daily_play_streak;
ALTER TABLE public.profiles RENAME COLUMN highest_play_streak    TO best_daily_play_streak;
ALTER TABLE public.profiles RENAME COLUMN current_solve_streak   TO current_daily_solve_streak;
ALTER TABLE public.profiles RENAME COLUMN best_solve_streak      TO best_daily_solve_streak;
...
BEGIN
  ...
  base AS (
    SELECT prof.id AS user_id,
      ...
      COALESCE(prof.current_daily_play_streak, 0)::INT AS current_streak,
      COALESCE(prof.best_daily_play_streak, 0)::INT AS highest_streak,
...
BEGIN
  ...
  SELECT
    ...
    a.instant_wins,
    a.flawless_wins,
    COALESCE(p.current_daily_play_streak, 0)::INT
  FROM agg a
...
BEGIN
  IF v_uid IS NULL THEN RETURN NULL; END IF;
  SELECT COALESCE(current_daily_play_streak,0), COALESCE(best_daily_play_streak,0), COALESCE(streak_freezes,0)
...
BEGIN
  ...
  SELECT current_daily_play_streak, best_daily_play_streak, last_daily_win_date, last_daily_play_date
  ...
  IF p_won THEN
    ...
    UPDATE public.profiles SET
      current_daily_play_streak = v_current_streak,
      best_daily_play_streak = v_highest_streak,
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(prof.current_daily_play_streak, 0)::INT AS play_streak,
    COALESCE(prof.best_daily_play_streak, 0)::INT AS highest_play_streak,
